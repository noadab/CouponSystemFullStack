package com.CouponSystemSpring.helpers;
...
public class SimpleTokenManager {
	...
	/**
	 * initializing a timer thread that will operate each
	 * EXPIRATION_THREAD_PERIOD_IN_MILLIS millis
	...
	public void initThread() {
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			public void run() {
				...
				if (tokens.size() == 0) {
					System.out.println("No active sessions, Tokens cleaner has stopped");
					timer.cancel();
				}
			}
	...
	public class Token {
		...
		public Token() {
			super();
			this.token = "Coupon_project_token_" + lastToken++;
